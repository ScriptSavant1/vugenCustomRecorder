...
**This should no longer happen.** Script Studio automatically handles any request header whose name contains `csrf`, `xsrf`, `antiforg`, or similar using one of two strategies depending on the application's CSRF pattern:

**Strategy 1 — Double-submit cookie** (most common: Laravel, Angular, Spring, ASP.NET):
...
Some dynamic values could not be automatically traced to their source. This only happens for values that are neither cookie-based, nor a recognisable UUID/hex token format, AND where the source response was not captured. Common causes:

- The HAR file was saved without response bodies (check you selected "with content" / "Include response bodies" in DevTools)
- The value comes from a redirect chain that wasn't captured
- The value is an opaque custom format the tool couldn't recognise

> **Note:** CSRF/XSRF headers and UUID/hex tokens are handled automatically and will NOT appear as TODO. If you see TODO for a non-CSRF body value, re-recording with DevTools **Disable Cache** checked usually resolves it.
